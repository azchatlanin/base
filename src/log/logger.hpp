...
*/
#define INFO_LOG(...)   pr::logger::log::info(LOGGER___TRACE_ON, __VA_ARGS__)
#define INFO_WARN(...)  pr::logger::log::warn(LOGGER___TRACE_ON, __VA_ARGS__)
#define INFO_ERROR(...) pr::logger::log::error(LOGGER___TRACE_ON, __VA_ARGS__)

namespace pr::test { inline void process() {}; };

namespace pr::logger
{
  template<class T, class charT = char, class traits = std::char_traits<char>>
  class log_ostream_iterator : public std::iterator<std::output_iterator_tag, void, void, void, void>
  {
   private:
  ...
  class log
  {
   ...
   public:
    ...
    static void print_v(const T& data)
    {
      ...
      std::copy(std::cbegin(data), std::cend(data), log_ostream_iterator<typename T::value_type>(std::cout));
    }
  ...
  inline void process() 
  {
    ...
    std::vector<std::string> vs { "asdf", "qwer", "zcxv" };
    INFO_LOG("vector", 1, vs);

    std::list<int> li { 1, 2, 3 };
    INFO_LOG("list", 2, li);

    INFO_LOG(1, 2.2, 3.32, "my string");
